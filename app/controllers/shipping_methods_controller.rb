class ShippingMethodsController < ApplicationController
  ...
  def execute
    ...
    if @import.valid?
      AddMultipleCorreiosZipcodeJob.perform_later(@shop.id, import_params)
      redirect_to import_shop_shipping_methods_path(@shop), notice: "Processo de importacao iniciado. Voce sera alertado quando a importacao for finalizada."
    else
      render :import
    end
  end
  ...
  def import_params
    params.require(:correios_calculate).permit(
      :service_code, :delivery_type, :zipcode_start, :safety_margin, :enterprise_code, :enterprise_pass
    )
  end
end
